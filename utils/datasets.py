...
from utils.paddle_utils import paddle_distributed_zero_first
...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    with paddle_distributed_zero_first(rank):
    ...
    sampler = DistributedBatchSampler(dataset, batch_size, drop_last=False, shuffle=False)
...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            newpath='./crop_150.jpg'
            img0 = cv2.imread(self.mypath)  # BGR
            print('路径是'+path)
            print(img0)
...
class LoadImagesAndLabels(Dataset):
    ...
    def collate_fn4(batch):
        ...
        ho = np.array([[0., 0, 0, 1, 0, 0]], dtype=np.float32)
        wo = np.array([[0., 0, 1, 0, 0, 0]], dtype=np.float32)
        s = np.array([[1, 1, .5, .5, .5, .5]], dtype=np.float32)  # scale
        for i in range(n):  # zidane np.zeros((16,3,720,1280), dtype=np.float32)  # BCHW
            i *= 4
            if random.random() < 0.5:
                im = cv2.resize(img[i].transpose(1,2,0), (0, 0), fx=2, fy=2, interpolation=cv2.INTER_LINEAR).transpose(2,0,1)
                l = label[i]
            else:
                im = np.concatenate((np.concatenate((img[i], img[i + 1]), 1), np.concatenate((img[i + 2], img[i + 3]), 1)), 2)
        ...
        return np.stack(img4, 0), np.concatenate(label4, 0), path4, shapes4

...
# Settings
paddle.set_printoptions(precision=5)
...
def get_latest_run(search_dir='.'):
    # Return path to most recent 'last.pdparams' in /runs (i.e. to --resume from)
...
def check_suffix(file='yolov5s.pdparams', suffix=('.pdparams',), msg=''):
...
def check_file(file, suffix=''):
    ...
    if Path(file).is_file() or file == '':  # exists
        ...
        paddle.hub.download_url_to_file(url, file) # 注意
        ...
        if not all(x.exists() for x in val):
            ...
            if s and autodownload:  # download script
                ...
                if s.startswith('http') and s.endswith('.zip'):  # URL
                    ...
                    paddle.hub.download_url_to_file(s, f) # 注意
    ...
    def download_one(url, dir):
        ...
        if Path(url).is_file():  # exists in current path
            ...
            if curl:
                ...
                paddle.hub.download_url_to_file(url, f, progress=True)  # paddle download 注意
...
def clip_coords(boxes, shape):
    ...
    if isinstance(boxes, paddle.Tensor):  # faster individually
...
# dict_keys(['epoch', 'best_fitness', 'updates', 'state_dict', 'optimizer', 'wandb_id'])
# dict_keys(['epoch', 'best_fitness', 'updates', 'state_dict', 'optimizer', 'wandb_id'])
def strip_optimizer(f='best.pdparams', s=''):  # from utils.general import *; strip_optimizer()
    # Strip optimizer from 'f' to finalize training, optionally save as 's'
    x = paddle.load(str(f))
    for k in 'optimizer', 'wandb_id', 'updates':  # keys
        x[k] = None
    x['epoch'] = -1
    paddle.save(x, str(s) or str(f))
    mb = os.path.getsize(s or f) / 1024 / 1024  # filesize
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(paddle.to_tensor(ims)).argmax(1)  # classifier prediction
...
def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    xyxy = paddle.to_tensor(xyxy).reshape([-1, 4])
    b = xyxy2xywh(xyxy)  # boxes
    if square:
        b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # attempt rectangle to square
    b[:, 2:] = b[:, 2:] * gain + pad  # box wh * gain + pad
    #xyxy = xywh2xyxy(b).long()
    xyxy = xywh2xyxy(b).astype(paddle.int32)
    x1, y1, x2, y2 = xyxy[0].numpy().astype(int)
    print("左上角坐标：({}, {})".format(x1, y1))
    print("右下角坐标：({}, {})".format(x2, y2))
